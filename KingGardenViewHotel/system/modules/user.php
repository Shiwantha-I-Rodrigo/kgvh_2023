...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0 && $email != $row['Email']) {
        ...
        $_SESSION['alert_msg'] = 'The email address provided has an account associated,<br> please <a href="/system/modules/login.php">log in</a> to continue, or use another email address.';
        ...
        if ($result->num_rows > 0 && $user_name != $row['UserName']) {
            ...
            $_SESSION['alert_icon'] = "error";
            $_SESSION['alert_title'] = "Error";
            $_SESSION['alert_msg'] =  'The username provided has an account associated,<br> please <a href="/system/modules/login.php">log in</a> to continue, or use another username.';
            reDirect('/system/sub/alert.php');
        } else {
            $upload = "";
