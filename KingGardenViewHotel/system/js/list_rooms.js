if (document.querySelector("#removed")) {
    Swal.fire({
        icon: "success",
        text: "The room is removed succesfully !",
        customClass: {
            popup: 'sw-alert',
        ...
        success: function (response) {
            ...
            for (i = 0; i < edits.length; ++i) {
                edits[i].addEventListener("click", function (event) {
                    window.location.replace("/system/modules/edit_rooms.php?id=" + this.id);
                })
            }
...
function gen_sql() {
    sql = '';
    search.value != "" ? sql += ` WHERE CONCAT_WS("_",i.RoomId,i.RoomName,i.RoomPrice) LIKE '%${search.value}%'` : sql += "";
    if (range.value != '') {
