if (document.querySelector("#removed")) {
    Swal.fire({
        icon: "success",
        text: "The invoice is removed succesfully !",
        customClass: {
            popup: 'sw-alert',
        ...
        success: function (response) {
            ...
            for (i = 0; i < edits.length; ++i) {
                edits[i].addEventListener("click", function (event) {
                    window.location.replace("/system/modules/edit_invoices.php?id=" + this.id);
                })
            }
...
function gen_sql() {
    sql = '';
    search.value != "" ? sql += ` WHERE CONCAT_WS("_",i.ItemId,i.ReservationId,i.ItemName,i.ItemPrice,i.ItemPaid,i.ItemDiscount,i.ItemComments) LIKE '%${search.value}%'` : sql += "";
    if (range.value != '') {
