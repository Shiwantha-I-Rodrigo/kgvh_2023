if (document.querySelector("#removed")) {
    Swal.fire({
        icon: "success",
        text: "The review is removed succesfully !",
        customClass: {
            popup: 'sw-alert',
        ...
        success: function (response) {
            ...
            for (i = 0; i < edits.length; ++i) {
                edits[i].addEventListener("click", function (event) {
                    window.location.replace("/system/modules/edit_reviews.php?id=" + this.id);
                })
            }
...
function gen_sql() {
    sql = '';
    search.value != "" ? sql += ` WHERE CONCAT_WS("_",i.ReviewId,i.ReservationId,i.ReviewTitle,i.ReviewText) LIKE '%${search.value}%'` : sql += "";
    if (range.value != '') {
