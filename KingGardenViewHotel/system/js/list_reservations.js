if (document.querySelector("#removed")) {
    Swal.fire({
        icon: "success",
        text: "The Reservation is removed succesfully !",
        customClass: {
            popup: 'sw-alert',
        ...
        success: function (response) {
            ...
            for (i = 0; i < edits.length; ++i) {
                edits[i].addEventListener("click", function (event) {
                    window.location.replace("/system/modules/edit_reservations.php?id=" + this.id);
                })
            }
...
document.getElementById("range").addEventListener("change", function (event) {
    ...
    if (this.value == 'r.TimeSlotStart' || this.value == 'r.TimeSlotEnd') {
...
function gen_sql() {
    sql = '';
    search.value != "" ? sql += ` WHERE CONCAT_WS("_",r.ReservationId,r.GuestId,r.StaffId,r.RoomId,r.TimeSlotStart,r.TimeSlotEnd,r.ReservationStatus,t.Price,t.Paid) LIKE '%${search.value}%'` : sql += "";
    if (range.value != 'r.TimeSlotStart' || range.value != 'r.TimeSlotEnd') {
