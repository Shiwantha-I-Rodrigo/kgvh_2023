        ...
        success: function (response) {
            ...
            for (i = 0; i < edits.length; ++i) {
                edits[i].addEventListener("click", function (event) {
                    window.location.replace("/system/modules/edit_blogs.php?id=" + this.id);
                })
            }
...
function gen_sql() {
    sql = '';
    search.value != "" ? sql += ` WHERE CONCAT_WS("_",i.BlogId,i.BlogText,i.BlogTitle) LIKE '%${search.value}%'` : sql += "";
    if (range.value != '') {
